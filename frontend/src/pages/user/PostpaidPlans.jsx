...
function PostpaidPlans() {
    ...
    const filteredPlans = selectedCategory ? plans.filter(plan => plan.planType.toLowerCase() === selectedCategory.toLowerCase() && plan.planCategory=== "postpaid" && plan.operator.toLowerCase()===operator.toLowerCase()) : plans.filter(plan=> plan.planCategory === "postpaid" );
    const clearFilter = () => {
        setSelectedCategory('');
    }
