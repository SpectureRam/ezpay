    ...
    try {
      ...
      if (response.data.messege === 'Login Success') {
        ...
        if(data){
          ...
          if(data.role === 'ADMIN'){
            SetMsg('Switching to Admin' );
            SetMsgclr('bg-sec');
            SetLoading1(false)
            setTimeout(()=>{
              SetMsg(""); 
              navigate('/EZpay/admin/dashboard'); 
            },2000)
          }
          else{
            SetMsgclr('bg-sec');
      ...
      else if (response.data.messege=='User account is suspended') {
        SetMsgclr('bg-red-500');
        SetLoading1(false)
        SetMsg('Your account is suspended. Please contact support for assistance.');
      }
      else {
